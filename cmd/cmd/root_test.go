...
func TestRegister(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rCmd := rootCmd{}
			rCmd.Register(tt.args.app, tt.args.configBucket, tt.args.envPrefix)
			...
			rCmd.logger = logger
			assert.NotNil(t, rCmd.config, "Config must not be nil")
			assert.Equal(t, zerolog.TimestampFieldName, "t", "Logger must be configured")
			rCmd.logger.Log("test message", tt.name)
			// assert.Equal(t, buf.String(),
			// fmt.Sprintf("{\"m\":\"test message: %s\"}\n", tt.name),
			// "Log message must be correct")
			// fmt.Printf("rCmd.config:\n%+v\n", rCmd.config)
			assert.Equal(t, "sara", *rCmd.config.SelfName, "Service name must be equal to 'sara'")
			assert.Equal(t, true, *rCmd.config.TLS, "Service TLS must be 'true'")
			assert.NotNil(t, rCmd.config.StoreConfig, "rCmd.config.StoreConfig must not be nil")
