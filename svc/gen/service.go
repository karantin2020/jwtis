...
func NewServerService(keysrepo *jwtis.KeysRepository, contEnc jose.ContentEncryption, log log.Logger) ServerService {
	if keysrepo == nil {
...
func (s *serviceImpl) UpdateKeys(ctx context.Context, req *UpdateKeysRequest) (*UpdateKeysResponse, error) {
	...
	resp, err := s.keysRepo.NewKey(kid, &jwtis.DefaultOptions{
		SigAlg:          req.SigAlg,
		SigBits:         req.SigBits,
		EncAlg:          req.EncAlg,
		EncBits:         req.EncBits,
...
func convertKeysOptions(opts *RegisterRequest) *jwtis.DefaultOptions {
